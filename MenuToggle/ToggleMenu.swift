...
class ToggleMenu: UIView {
    ...
    /**
     The effect style of the menu background
     */
    ...
    var toggled = false
    /**
     The image of the toggleButton when initialized as an IBoutlet in storyboard/XIB
     */
    ...
    init(frame: CGRect, toggleImage: UIImage, menuButtons: [UIButton], withEffectStyle:UIBlurEffectStyle) {
        ...
        self.blurEffectStyle = withEffectStyle
    }
    ...
    private func layoutMenu(){
        if !hasLayedOutView{
            ...
            if self.blurEffectStyle == nil{
                ...
                blurEffect = UIBlurEffect(style: self.blurEffectStyle!)
            }
            let blurEffectView = UIVisualEffectView(effect: blurEffect)
            blurEffectView.frame = self.bounds
            blurEffectView.layer.cornerRadius = self.bounds.width/2
    ...
    func removeConstraints(){
        ...
        while (superview != nil) {
            ...
            for c in superview!.constraints{
                if c.firstItem is ToggleMenu{
                    superview!.removeConstraint(c)

                }else if c.secondItem is ToggleMenu {
                    superview!.removeConstraint(c)
                }
            }
